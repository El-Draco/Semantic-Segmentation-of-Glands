...
def predict(img,
    ...
    with torch.no_grad():
        out = model(x)
        #out=out['out']
        #print(f"Image shape:{out.shape}")  # send through model/network
...
# model
model = SwinUNETR(img_size=(imgsize,imgsize), in_channels = imgchant, out_channels = numclasses, spatial_dims=2).to(device)


model_name = 'SwinUNETR_epochs_adam_actual_ftl.pt'
...
plot_comparison(ground_truths_to_plot, predictions_to_plot, save_path='ground_truth_vs_predictions.png')
