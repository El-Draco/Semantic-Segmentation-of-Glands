...
# from sklearn.externals import bytescale
...
def re_normalize(inp: np.ndarray,
    ...
    if min_val == max_val:
        # Handle case when min_val and max_val are equal
        inp_out = np.full_like(inp, (low + high) / 2)  # Fill with mean of low and high
    else:
        inp_out = (inp - min_val) * (high - low) / (max_val - min_val) + low
